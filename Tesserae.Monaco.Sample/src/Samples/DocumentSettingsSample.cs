            ...
            _content = SectionStack().Secondary()
               ...
               .FlatSection(VStack().Children(
                    Card(VStack().WS().Children(
                        ...
                        TextBlock("Open search.cs and press the Settings button on the strip (or Ctrl+comma, or Ctrl+P and pick it under Settings). Change the path: the button turns brand-coloured and says \"1 change\", the chip shows the pending value, the tab gets its unsaved dot, and the banner names what is waiting. Close the overlay - the edits stay. Hover the tab to read what is unsaved. Type in the editor too and the close prompt says both changed. Ctrl+S saves them together and everything comes clean; Revert settings puts the values back and rebuilds the form. nightly-import.cs builds its form with PropertyGrid instead, and readme.md has no settings at all, so it gets no strip."),
