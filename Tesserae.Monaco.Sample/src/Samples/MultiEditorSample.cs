            ...
            _content = SectionStack().Secondary()
               ...
               .FlatSection(VStack().Children(
                    Card(VStack().WS().Children(
                        ...
                        TextBlock("A document opens once. .Open(id) on a document already in a tab brings that tab to the front and puts the caret in its editor rather than building a second one, so a tree row, Ctrl+P and a host's own button all lead to the same editor - the document's id is its identity. .Reveal(id) is that half on its own, for bringing an open document forward without opening one that is not.").MT(8),
